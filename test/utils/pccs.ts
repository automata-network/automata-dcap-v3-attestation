...
export async function setupPccs(): Promise<SetupReturnType> {
    // Strp 1: re-deploy the helper contract because they are outdated
    const enclaveIdHelper = await ethers.deployContract("EnclaveIdentityHelper");
    const enclaveIdHelperRet = await enclaveIdHelper.getAddress();
    console.log(`EnclaveIdHelperAddr deployed at: ${enclaveIdHelperRet}`);
    const fmspcTcbHelper = await ethers.deployContract("FmspcTcbHelper");
    const fmspcTcbHelperRet = await fmspcTcbHelper.getAddress();
    console.log(`FmspcTcbHelperAddr deployed at: ${fmspcTcbHelperRet}`);
    const pcsDao: PcsDao = await ethers.getContractAt("PcsDao", PCS_DAO_PORTAL!);
    
    // Step 2: upsert root ca
    ...
    // step 6: upsert enclave identity
    ...
    return {
        enclaveIdHelperAddr: enclaveIdHelperRet,
        fmspcTcbHelperAddr: fmspcTcbHelperRet
    }
}
